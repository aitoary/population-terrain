...
- 操作パネルは選択メッシュの詳細・人口推移・共有を先頭に配置。「注目地点」は地点名・増減率・短い説明の3択で、選ぶと再生を停止して2070年の対象メッシュへ移動します。共有は選択後に詳細内の「この表示をコピー」を使います。[選定理由・全11年の値](docs/featured-locations.md)。
...
npm run test:guide # コンパクトな3択・パネル順序・再生停止・詳細からの共有（300秒上限）
